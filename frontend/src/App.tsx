...
import { type BftSnapshot, getBft } from "./api";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        const snap = await getBft(persona.phone);
        if (!cancelled) setBft((b) => ({ ...b, [persona.phone]: snap }));
      } catch {
        ...
        <div className="sidebar">
          {PERSONAS.map((p) => {
            ...
            return (
              <button
                key={p.phone}
                className={`persona-card ${p.phone === persona.phone ? "active" : ""}`}
