...
| Speech (STT/TTS), OCR | faster-whisper, Piper-TTS, PaddleOCR | Unchanged (Phase 2) | Free, open weights, run locally. |
...
| Event backbone | Kafka + Confluent Schema Registry + Flink | **In-process async event bus** for now; **Redpanda** (Kafka-API compatible, single binary, free) documented as the Phase 3 swap | Kafka's own operational footprint (plus Zookeeper, plus a schema registry, plus Flink) is disproportionate to what a two-person team needs before there's real multi-service load. The event contract is written so swapping the transport doesn't touch agent or orchestrator code. |
...
| Federated learning / DP | Flower, Opacus | Unchanged (Phase 3) | Free, open source — just deferred until there are enough simulated clients for federation to mean anything. |
...
| Container orchestration | Kubernetes | `docker compose` for the prototype; k8s manifests scaffolded, not run | Free either way — Kubernetes' value shows up at a scale this repo isn't at yet. |
...
| **Setu Account Aggregator** | Setu AA SDK | **Mocked**, real sandbox credentials documented as a Phase 3 TODO | Setu does offer a free developer sandbox — this one is realistic to wire up for real later, just not done yet. |
