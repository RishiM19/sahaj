...
| **Cash Flow** | Runs a 10,000-trial Monte Carlo income/expense projection with structured seasonality (crop calendar, gig demand cycle, MNREGA) and flags upcoming deficits. | Phase 1 (hand-encoded seasonality, not a model trained on real gig/crop-yield data - see `app/agents/seasonal.py`) |
| **Life Simulator** | Turns the same Monte Carlo run into a spoken "what happens next" narrative (FLSE). | Phase 1 (shares Cash Flow's engine, so the numbers it narrates always agree with what Cash Flow warned about) |
